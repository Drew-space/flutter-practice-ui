...
class TMDBService {
  ...
  // ── Fetch Season Episodes (with still_path per episode) ──
  Future<List<Map<String, dynamic>>> fetchSeasonEpisodes(
    int seriesId,
    int seasonNumber,
  ) async {
    final url =
        '$_baseUrl/tv/$seriesId/season/$seasonNumber?api_key=$apikey&language=en-US';
    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      final episodes = data['episodes'] as List<dynamic>? ?? [];
      return episodes.map((ep) => ep as Map<String, dynamic>).toList();
    }
    throw Exception('Failed to load season episodes: ${response.statusCode}');
  }
