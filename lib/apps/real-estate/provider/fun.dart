...
class FavoritesNotifier extends Notifier<Set<String>> {
  // This box was already opened once in main.dart, before the
  // app started. Here I'm just grabbing a reference to that
  // SAME box so I can read from it and write to it.
  Box get favouritesBox => Hive.box("favouritesBox");

  // This runs once when the app first asks for the favorites.
  // Instead of starting empty every time, I read whatever was
  // saved last time from Hive.
  @override
  Set<String> build() {
    // 🔵 DEBUG — remove these print lines once everything works
    print("🔵 build() running...");
    print("🔵 Is favouritesBox open? ${Hive.isBoxOpen('favouritesBox')}");

    // Hive can only store simple things like Lists, not Sets
    // directly, so I saved it as a List and I convert it back
    // to a Set here.
    List<dynamic> savedList = favouritesBox.get(
      "favouriteIds",
      defaultValue: <String>[],
    );

    print("🔵 Loaded from Hive on startup: $savedList");

    return Set<String>.from(savedList);
  }
  ...
  void toggle(String houseId) {
    print("🟡 toggle() called for houseId: $houseId");
    ...
    // Step 1: update the Riverpod state so the UI
    // (the heart icon, the favorites screen) updates immediately.
    state = updatedFavorites;

    // Step 2: save the new list to Hive so it's still there
    // next time the app opens, even with no internet.
    favouritesBox.put("favouriteIds", updatedFavorites.toList());

    // 🟢 DEBUG — confirm it actually landed in the box
    print("🟢 Saved to Hive: ${updatedFavorites.toList()}");
    print("🟢 Reading it straight back: ${favouritesBox.get('favouriteIds')}");
  }
